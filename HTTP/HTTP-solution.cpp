...
// Copy constructor.
HTTP::HTTP( const HTTP &existing ) :
    host_name( existing.host_name ),
    port_number( existing.port_number ),
    socket_handle( -1 )
{
    // Make a new connection to the same host/port as the existing connection.
    establish_connection( );
}


// Copy assignment first cleans up *this and then copies the existing connection.
HTTP &HTTP::operator=( const HTTP &existing )
{
    if( this != &existing ) {
        if( socket_handle != -1 ) {
            close( socket_handle );
...
HTTP::HTTP( HTTP &&existing ) :
    host_name( std::move( existing.host_name ) ),
    port_number( existing.port_number ),
    socket_handle( existing.socket_handle )
{
    // The existing object no longer "owns" this connection.
    existing.socket_handle = -1;
}


// Move assignment "drains the existing object after cleaning up *this.
HTTP &HTTP::operator=( HTTP &&existing )
{
    if( this != &existing ) {
        if( socket_handle != -1 ) {
            close( socket_handle );
